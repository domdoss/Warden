...
GROUPS = [
    ...
    {"id": "signal", "label": "When to trade",
     "help": "The signal is the ticker's own move over the horizon window. These knobs decide how strong that move must be before the engine trades it, and whether it may short."},
...
KNOBS = [
    ...
    {"key": "horizon_min", "group": "signal", "label": "Trade horizon", "type": "select", "default": 30,
     ...
     "help": "The signal is the ticker's move over this many minutes; a trade's stop/target are sized on the typical move over the same window and it is time-exited after it. 30 min is the starting point."},
    {"key": "edge_mult", "group": "signal", "label": "Signal strength needed", "type": "float", "default": 1.0,
     "min": 0.5, "max": 5.0, "step": 0.1, "unit": "× cost",
     "help": "The move must be bigger than this many times the round-trip cost before the engine acts on it. 1 = the move must at least pay for the trade. Higher = fewer, more confident trades."},
