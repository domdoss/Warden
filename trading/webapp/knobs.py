...
KNOBS = [
    ...
    {"key": "lt_align", "group": "align", "label": "Trade with the long-term stance", "type": "select",
     ...
     "help": "The stance comes from the TradingAgents rating first, then the Liquid NN daily call. The engine's own walk-forward evidence still decides which tickers may trade at all; this setting only shapes the direction. Default off: the intraday models trade on their own measured evidence, and the stance is shown for context only."},
