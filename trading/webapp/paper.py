...
# ── Unified autotrade switch ──────────────────────────────────────────────────
# Two switches, in state.json settings, selecting WHO auto-trades this shared
# account: autotrade_daytrade (the day-trade engine trades its calls) and
# autotrade_longterm (TradingAgents decisions place orders that fill at the
# next market open). A source switched off still shows its calls / logs its
# decisions — it just doesn't execute. Manual trades are always allowed
# (the human is not a source). Defaults match the pre-switch behavior:
# day trader signals only, long-term calls execute.
_STATE_PATH = Path(CONFIG.get("state_file",
                              str(Path.home() / ".alpha-stack" / "state.json"))).expanduser()


def trade_sources() -> set[str]:
    """The sources allowed to auto-trade, as a set: may contain "daytrade"
    and/or "longterm" (empty = nothing auto-trades, manual only)."""
    try:
        ...
        settings = {}
    out = set()
    if settings.get("autotrade_daytrade", False):
        out.add("daytrade")
    if settings.get("autotrade_longterm", True):
        out.add("longterm")
    return out
