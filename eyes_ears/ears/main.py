...
from ears.assistant import WardenBridge
from core.config import Config
from ears.session_store import SessionStore
from ui.jarvis_window import JarvisWindow
from ears.audio import AudioPlayer, AudioRecorder, BeepGenerator, find_device
from ears.capture import capture
from ears.clap import ClapDetector, SatelliteClapDetector
from ears.satellite import SatelliteAudioPlayer, SatelliteAudioRecorder
from ears.stt import STT
from ears.tts import TTS
...
class JarvisApp:
    def __init__(self, mic_host: Optional[str] = None,
                 ...
                 ui_only: bool = False):
        self.config = Config()
        self.bridge = WardenBridge(config=self.config)
        self._control_port = control_port
        # UI-only mode: render the same window + Warden bridge (text chat +
        # dashboard panels) WITHOUT any audio plumbing — no mic/speaker/STT/
        # TTS/clap/control-server/hotkey. Same HTML, no audio routing. Lets a
        # machine run just the UI against the Warden server.
        ...
        if self._ui_only:
            # UI-only: no mic/speaker/STT/TTS — the window + Warden bridge are
            # all that's needed for text chat and the dashboard panels.
            ...
            self._setup_audio(mic_host, speaker_host, voice_cfg, audio_cfg,
                              sat_cfg, sat_port)
    ...
    # ----- audio setup (skipped entirely in UI-only mode) ------------------
    def _setup_audio(self, mic_host, speaker_host, voice_cfg, audio_cfg,
                     sat_cfg, sat_port):
        # ── mic side ───────────────────────────────────────────────────────
        self._input_device = None
        self._sat_mic_host = None
    ...
    def _get_warden_url(self) -> str:
        return self.config.warden.get("base_url", "") or ""

    def _save_warden_url(self, url: str) -> None:
        """Persist warden.base_url and apply it to the live bridge client.
        ...
        """
        url = (url or "").strip()
        self.config.set("warden.base_url", url)
        self.config.save()
        try:
            self.bridge.client.base_url = url.rstrip("/")
        except Exception as e:
            print(f"[jarvis] apply warden url to bridge failed: {e}")
        print(f"[jarvis] saved warden.base_url={url} (restart Jarvis to rebind the SSE stream)")
    ...
    def _ensure_configured(self) -> bool:
        ...
        if not self.config.warden.get("base_url"):
    ...
    async def _bootstrap_async(self):
        ...
        if self._ui_only:
            # No TTS worker, no spoken greeting — the SSE stream is still
...
def main():
    ...
    # Audio source is config-driven: the only source of the satellite host is
    # config satellite.host (set via './run.sh --configure'). No hardcoded IP,
    # no local fallback — if a bare 'remote' is requested and satellite.host
    # isn't configured, refuse to run rather than guess.
    cfg = Config()
    sat_host = (cfg.get("satellite", {}) or {}).get("host") or ""

    def _need_sat(reason: str) -> str:
        if not sat_host:
            raise SystemExit(
                f"[ears] {reason}: satellite.host is not configured. "
                "Run './run.sh --configure' (Satellite → host) first.")
        return sat_host

    def _parse_side(val: Optional[str]) -> Optional[str]:
        """Resolve a --mic/--speaker value to a remote host, or None for local.

        'local' → None; 'remote' or 'remote:' (no host) → configured
        satellite.host (error if unset); 'remote:HOST' → HOST; a bare
        host/ip → remote at that host."""
        if val is None:
            return None  # not specified → the no-flag path decides
        v = val.strip()
        if v == "local":
            return None
        if v == "remote" or v == "remote:":
            return _need_sat("bare 'remote' audio")
        if v.startswith("remote:"):
            return v[len("remote:"):].strip() or _need_sat("'remote:' audio")
        return v  # bare host/ip → remote

    p = argparse.ArgumentParser(description="Jarvis — Warden thin voice client")
    ...
    p.add_argument("--remote", metavar="HOST", nargs="?", const="", default=None,
                   help="Shorthand for --mic remote:HOST --speaker remote:HOST. "
                        "Bare --remote uses the configured satellite.host.")
    ...
    p.add_argument("--set-warden-url", metavar="URL", default=None,
                   help="Write warden.base_url to the user config and exit, "
                        "without launching the UI. Scriptable alternative to "
                        "the in-app settings field.")
    p.add_argument("--ui-only", action="store_true", default=False,
                   help="Launch just the UI (hologram + panels + text chat) with "
                        "NO audio: no mic/speaker/STT/TTS/clap/control-server/"
                        "hotkey. Same HTML as --ears, runs against the Warden "
                        "server. No --mic/--speaker/--remote needed.")
    args = p.parse_args()

    if args.set_warden_url is not None:
        cfg.set("warden.base_url", args.set_warden_url)
        cfg.save()
        print(f"[jarvis] saved warden.base_url={args.set_warden_url} → {cfg.config_path}")
        return

    if args.ui_only:
        # No audio source resolution — UI-only skips all mic/speaker/STT/TTS.
        JarvisApp(control_port=args.control_port, ui_only=True).run()
        return

    mic_host = _parse_side(args.mic)
    speaker_host = _parse_side(args.speaker)
    # --remote sets BOTH sides remote — but only for sides the caller didn't
    # set explicitly, so `--remote x --mic local` keeps the mic local.
    if args.remote is not None:            # bare --remote → const="" ; HOST → HOST
        rhost = args.remote or _need_sat("bare --remote")
        if args.mic is None:
            mic_host = rhost
        if args.speaker is None:
            speaker_host = rhost

    # No audio flag at all → use the configured satellite.host for both sides.
    # No local fallback: if it isn't configured, refuse to run.
    if args.mic is None and args.speaker is None and args.remote is None:
        mic_host = _need_sat("no audio source given")
        speaker_host = mic_host
