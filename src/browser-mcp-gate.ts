...
const TOOL_JSON: Record<string, Record<string, unknown>> = {
  ...
  chrome_read_page: { what: 'accessibility tree of visible page elements', use_when: 'find element refs before click/type', fallback: 'element missing → chrome_computer screenshot, act by coordinates' },
  ...
  chrome_javascript: { what: 'run JS in a tab and return the value', code: 'async function body — return x; await ok', tabId: 'omit = active tab', timeoutMs: 15000 },
  chrome_click_element: { what: 'click an element', target: 'ref (from chrome_read_page) | selector | coordinates', tabId: 'omit = active tab' },
  chrome_fill_or_select: { what: 'fill input/textarea/select/checkbox/radio', target: 'ref | selector', value: 'string | boolean | option', tabId: 'omit = active tab' },
