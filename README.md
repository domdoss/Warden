...
Delegation is not fire-and-forget. When the orchestrator hands work to Atlas, Atlas runs **in the background** — the orchestrator gets a job ID back immediately and stays free to handle your next message. While those jobs run, supervision happens on a **self-audit tick** (default every **10 minutes** — cadence is selectable in the dashboard Supervisor row, and the whole watchdog can be turned off there), in two layers:

**An advisory LLM check.** On each tick a small *supervisor* model (distinct from the orchestrator — by default the local toolcall model, e.g. `granite4.1:3b`) reads a one-line snapshot per running job — elapsed time, tool-call count, what it last did and how long ago — and tags it **on track**, **off-rails**, or **crashed**. The prompt is tuned to recognize **normal behaviour on local hardware**: a long elapsed time, a long streak of read-only calls (reading files, research), a high call count, and idle gaps while the model generates its next response are all normal progress, not problems — a single task routinely takes 10–30 minutes here. It only flags an **obvious loop** (the same call repeated verbatim with no progress) or **obvious veering** (calls serving a goal unrelated to the task). This verdict is **advisory only by design**: a one-line snapshot can't safely tell a dead job from a cloud-API stall, so the LLM never kills a job itself — it hands the flag to the orchestrator, which decides whether to `nudge_agent`, `stop_agent`, or let it run. (The old READ-HEAVY label that flagged healthy reads as waste is gone.)
...
- **Advisory-only watchdog** — the 10-min LLM self-audit (cadence selectable; can be disabled) flags obvious loops/veering but treats slow local-model reads as normal and never kills a job itself; the orchestrator decides whether to nudge or stop.
...
| **Supervisor (watchdog)** | Small local (e.g. granite4.1:3b) | Runs the 10-min self-audit tick + completion verdict — tool-less, a few hundred tokens per call. A small local model keeps it resident without churning VRAM. Falls back to the orchestrator model if unset. The tick cadence is selectable (default 10 min) and the watchdog can be turned off from the dashboard Supervisor row. |
