...
Delegation is not fire-and-forget. When the orchestrator hands work to Atlas, Atlas runs **in the background** — the orchestrator gets a job ID back immediately and stays free to handle your next message. While those jobs run, one layer of supervision is active:

**The LLM self-audit watchdog is currently disabled.** It was a periodic tick (default every 10 minutes) where a small *supervisor* model read a one-line snapshot per running job and tagged it **on track**, **off-rails**, or **crashed**, handing any flag to the orchestrator to `nudge_agent` / `stop_agent` / let it run. It is now **hard-disabled**: the ticker never arms, the tick function is a no-op, and the dashboard Supervisor row (model + on/off + cadence) is removed. On local hardware a 3b model could not reliably tell a slow read-or-compose from veering — it kept flagging healthy Atlas jobs as off-track (e.g. an Atlas that read the source file in chunks then went idle ~3min while composing the rewrite), and the standing escalation rule stopped the job and handed the work to Vulkan. It stays out until it's rebuilt to actually distinguish progress from veering. (The escalation plumbing itself is untouched; only the tick source is dead.)
...
Crucially, supervision stays out of your chat. The churn detector's commit nudge is injected into the agent's own turn, not your conversation. Progress lives in the dashboard instead: the real status line each job emits on every tool call streams into a **grouped, collapsible Live Activity panel** (one summary line when collapsed, the recent history when expanded), so you watch what's actually happening without a parade of chat bubbles. The chat only carries **completed-task reports** (and interventions) — you ask once, the orchestrator drives the whole chain end to end, and you hear from it when there's something finished to tell you.
...
- **Advisory-only watchdog (currently disabled)** — the 10-min LLM self-audit that flagged obvious loops/veering and let the orchestrator decide whether to nudge or stop. Hard-disabled after it kept mis-flagging healthy slow local-model reads as off-track (a 3b model can't reliably tell a long compose from veering); the ticker never arms and the dashboard Supervisor row is removed. Only the deterministic churn detector stops jobs now.
...
| **Supervisor (watchdog)** | Small local (e.g. granite4.1:3b) | **The running-job self-audit tick is disabled** (see above). The model still runs the **completion verdict** on finished jobs — tool-less, a few hundred tokens per call, falls back to the orchestrator model if unset. Not adjustable from the dashboard while the watchdog is disabled (the Supervisor row is removed). |
