    ...
    def _fetch_groq_tts(self, text, my_gen):
        ...
        with self.lock:
            if self.gen != my_gen:
                ...
                return None
        # Single attempt, short timeout. ANY Groq error — 429 rate limit,
        # network, auth, Cloudflare block, or timeout — gives up immediately
        # and lets speak() fall back to espeak-ng. No retries, no backoff,
        # no waiting around.
        try:
            ...
            sys.stderr.write("TTS fetch failed (falling back to espeak): %s\n" % e)
            return None
